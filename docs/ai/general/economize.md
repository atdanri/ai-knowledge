...
### regulation
There are various ways to harden tools against token wastage. For example, in most tools you can limit the maximum number of tokens per window (useful if the model you’re using goes haywire), in theory, you can use hooks to limit the file sizes for the context... Decide for yourself whether this makes sense for you
...
when tokens need to be saved, use freely available AI chatbots or a standard search engine for simple separate tasks and researches
