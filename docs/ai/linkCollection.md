# AI knowledge link collection
[🔙README](/README.md)

## AI-tools

### kiro
- [docs for IDE & CLI](https://kiro.dev/docs/)
- kiro MCP docs [IDE](https://kiro.dev/docs/mcp/) | [CLI](https://kiro.dev/docs/cli/mcp/)

### copilot
- [docs(en)](https://docs.github.com/en/copilot) 
- [docs(de)](https://docs.github.com/de/copilot)
- [Configure MCP servers for your repository](https://docs.github.com/en/copilot/how-tos/copilot-on-github/customize-copilot/configure-mcp-servers)

### chatbots
- [duck.ai](https://duck.ai/) - test different small LLMs(GPT mini, Haiku,...) - designed for cost-effectiveness
- Many other chatbots are available free of charge, with or without registration - Gemini(Google search), Copilot, ChatGPT, Deepseek,...

## general
- [promtcowboy - optimize prompts](https://www.promptcowboy.ai/)

### LLMs
- [openrouter.ai/compare](https://openrouter.ai/compare) - compare LLMs by key metrics...

### MCP
- [MCP docs](https://modelcontextprotocol.io/docs)
- [danvega tutorial for development of a mcp server with Java & Spring AI](https://www.danvega.dev/blog/creating-your-first-mcp-server-java)

### ACP
- [ACP docs](https://agentclientprotocol.com/get-started/introduction)

