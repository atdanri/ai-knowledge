# personal collection of AI knowledge
> ⚠️ **Attention:** no guarantee of absolute accuracy or that the content is still up to date!

Personal knowledge base and cheat sheet for AI development, prompt engineering, and context optimization,... Features configuration guides and insights for tools like GitHub Copilot and Kiro.


- [own AI experience - only some thoughts](docs/ai/experience.md)
- [some might be usefulexternal links](docs/ai/linkCollection.md)

## theory and use topics

- [economize - need fewer tokens](docs/ai/theory/economize.md)
- [prompting](docs/ai/theory/prompting.md)
- [good to know topics](docs/ai/theory/goodToKnow.md)
- [debugging and refactoring](docs/ai/theory/debuggingAndRefactoring.md)
- [testing](docs/ai/theory/testing.md)
- [other development topics](docs/ai/theory/otherDevelopment.md)

## tools
The focus here, in terms of tools, is on Kiro and Copilot

- [Agentic Coding Protocol - Integrating tools into IDEs](docs/ai/tools/acp.md)
- [Caveman - minimize chat communication and token consumption](docs/ai/tools/caveman.md)

### kiro
- [kiro default files & directories(primarily for spec-driven development)](docs/ai/tools/kiro/kiroFiles.md)
- [kiro CLI knowledge](docs/ai/tools/kiro/knowledge.md)
- [personal opinion on kiro](docs/ai/tools/kiro/opinion.md)

### copilot
- [copilot default files & directories](docs/ai/tools/copilot/copilotFiles.md)
- [personal opinion on copilot](docs/ai/tools/copilot/opinion.md)

### antigravity
- [antigravity default files & directories](docs/ai/tools/antigravity/antigravityFiles.md)
- [my unqualified opinion on Antigravity]()

---
> ❗*It is worth bearing in mind that there is currently a great deal of activity in the field of AI development tools. It is quite possible that the tools discussed here will soon no longer be among the top tools. More and more tools are becoming available, some of which are optimised for specific areas of development. The future is uncertain*
