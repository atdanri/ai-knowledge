...
### webapps
Many AI dev tools already come with agents or/and the necessary tools to run and analyse web applications. Testing front-end and back-end combinations should also be straightforward in most tools with the right prompts and the necessary context. If required, there are browser extensions and other tools(eg. Jam) that can be integrated with the actual AI tool(often with MCP-services).
...
## refactoring
AI development tools can assist you with refactoring in a variety of ways. Identifying code smells, anti-patterns... and even fixing them is usually not a major problem.
